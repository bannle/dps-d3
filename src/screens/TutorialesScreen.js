...
export default function TutorialesScreen() {
    ...
    useEffect(() => {
        setTutorialesFiltrados(recursos);
    }, [recursos]);
    ...
    const filtrarTutoriales = (texto) => {
        ...
        setTutorialesFiltrados(filtrados);
    };
    ...
    const obtenerRecursos = async () => {
        try {
            ...
            const tutoriales = response.data.filter((item) => item.tipo?.toLowerCase() === 'tutorial');
            setRecursos(tutoriales);
        } catch (error) {
    ...
    const renderItem = ({ item }) => (
        <View style={styles.card}>
            <Image
                source={{ uri: item.imagen?.url || item.imagen || 'https://cdn-icons-png.flaticon.com/512/3652/3652191.png' }}
                style={styles.image}
            />
            ...
            <TouchableOpacity onPress={() => abrirEnlace(item.enlace?.url || item.enlace)} style={styles.boton}>
                <Text style={styles.botonTexto}>Ver Tutorial</Text>
            </TouchableOpacity>
    ...
    return (
        <View style={{ flex: 1 }}>
            <TextInput
                ...
                value={busqueda}
                onChangeText={filtrarTutoriales}
                style={styles.barraBusqueda}
            />

            {tutorialesFiltrados.length === 0 ? (
                <View style={styles.sinRecursosContainer}>
                    <Text style={styles.sinRecursosTexto}>No hay tutoriales por el momento</Text>
                </View>
            ) : (
                <FlatList
